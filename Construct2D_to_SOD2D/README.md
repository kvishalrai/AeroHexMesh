...
A second case pushes into the transonic, shock-buffet regime: flow over
NACA0012 at `Re_c = 500`, `α = 10.0°`, `M_∞ = 0.8`:

![NACA0012 transonic validation: Mach contours and -Cp/Cf vs x/c, Present against Soldati et al. (2024)](validation_naca0012_transonic_re500_m08.png)

Mach contours (top: reference; bottom: present) show the same shock
structure and separated wake, and `-C_p`/`C_f` along the chord (Coarse
and Medium mesh resolutions) match the reference solution closely,
including the shock location and the leading-edge separation captured
in `C_f`.

- G. Soldati, A. Ceci, S. Pirozzoli, "FLEW: A DNS Solver for Compressible
  Flows in Generalized Curvilinear Coordinates," *Aerotecnica Missili &
  Spazio*, vol. 103, pp. 413–425, 2024.
  DOI: [10.1007/s42496-024-00199-4](https://doi.org/10.1007/s42496-024-00199-4)
